...
public class ZModelUserBehavior: ZModelBase {
    ...
    public var behavior_user_login: ZModelUserLogin?
    public var behavior_userid: String = ""
    public var behavior_time: Double = Date().timeIntervalSince1970
    ...
    public required init<T: ZModelBase>(instance: T) {
        ...
        if let user = model.behavior_user_login {
            self.behavior_user_login = ZModelUserLogin.init(instance: user)
        }
    ...
    public required init(row: Row) {
        ...
        if let _ = row["userid"] {
            ...
            self.behavior_user_login = ZModelUserLogin.init(row: row)
        }
