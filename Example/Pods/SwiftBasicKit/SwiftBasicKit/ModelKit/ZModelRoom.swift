...
public class ZModelRoom: ZModelBase {
    ...
    enum Columns: String, ColumnExpression {
        case room_ownerid, room_name, room_type, room_time, room_video, room_audio, room_cover, video_time, audio_time, is_online, online_count, is_like, like_count, play_count
    }
    public var room_owner: ZModelUserInfo?
    public var room_ownerid: String = ""
    public var room_name: String = ""
    public var room_type: zRoomType = .video
    public var room_time: Double = Date.init().timeIntervalSince1970
    ...
    public required init<T: ZModelBase>(instance: T) {
        ...
        if let owner = model.room_owner {
            self.room_owner = ZModelUserInfo.init(instance: owner)
        }
    ...
    public required init(row: Row) {
        ...
        self.room_owner = ZModelUserInfo.init(row: row)
