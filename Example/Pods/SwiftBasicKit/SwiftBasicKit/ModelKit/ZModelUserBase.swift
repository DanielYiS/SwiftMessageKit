...
public class ZModelUserBase: ZModelBase {
    ...
    public required init<T: ZModelBase>(instance: T) {
        ...
        guard let model = (instance as? ZModelUserBase) ?? (instance as? ZModelUserInfo) else {
            return
        }
    ...
    public override func mapping(mapper: HelpingMapper) {
        ...
        mapper <<< self.show_location <-- "settings.show_location"
        mapper <<< self.incognito_mode <-- "settings.incognito_mode"
        mapper <<< self.height <-- "profile.attrs.height"
        mapper <<< self.weight <-- "profile.attrs.weight"
        mapper <<< self.bodytype <-- "profile.attrs.bodytype"
        mapper <<< self.belong <-- "profile.attrs.belong"
    }
}
