...
public struct ZAlertView {
    ...
    /// 显示多个ActionSheet
    public static func showActionSheetView(vc: UIViewController? = nil, title: String? = nil, message: String? = nil, buttons: [String], completeBlock: ( (_ index: Int) -> Void)?) {
    ...
    /// 显示多个ActionSheet
    public static func showActionSystemSheetView(vc: UIViewController? = nil, title: String? = nil, message: String? = nil, buttons: [String], completeBlock: ( (_ index: Int) -> Void)?) {
        
        let itemVC = ZAlertSystemViewController.init(title: title, message: message, preferredStyle: .actionSheet)
        for (row, btn) in buttons.enumerated() {
            let action = UIAlertAction.init(title: btn, style: UIAlertAction.Style.default) { (action) in
                completeBlock?(action.tag)
            }
            action.tag = row
