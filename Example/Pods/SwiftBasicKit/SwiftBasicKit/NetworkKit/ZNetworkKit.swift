...
public struct ZNetworkKit {
    ...
    /// - returns: 返回值描述
    @discardableResult
    public func startRequest(isShowHud: Bool = false, target: ZNetworkTargetType, responseBlock: @escaping ((_ result: ZNetworkResult) -> (Void))) -> Cancellable {
        self.typePlugin.isShowHud = isShowHud
        ...
        let task = provider.request(target, progress: nil, completion: { (result) in
            ...
            case .success(let response):
                do {
                    ...
                    case ZNetworkCode.unAuthorized: NotificationCenter.post(name: Notification.Names.LoginExpired)
                    case ZNetworkCode.balance: NotificationCenter.post(name: Notification.Names.InsufficientBalance)
                    default: break
                    }
