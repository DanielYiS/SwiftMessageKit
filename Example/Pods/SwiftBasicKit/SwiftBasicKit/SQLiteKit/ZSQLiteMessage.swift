        ...
        do {
            try ZSQLiteKit.shared.connection?.write({ db in
                if ZSettingKit.shared.role == .anchor {
                    ...
                    let row = try Row.fetchOne(db, sql: "SELECT COUNT(t1.message_id) as unReadCount FROM tb_message t1 LEFT JOIN tb_message_user t2 ON t1.message_userid = t2.userid WHERE t1.login_userid = ? AND t1.message_read_state = 0 ", arguments: [ZSettingKit.shared.userId])
                    count = Int(row?["unReadCount"] as? Int64 ?? 0)
                }
            })
        } catch {
