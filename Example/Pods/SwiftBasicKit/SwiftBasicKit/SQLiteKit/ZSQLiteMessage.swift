...
extension ZSQLiteKit {
        ...
        do {
            ...
            try ZSQLiteKit.shared.connection?.write({ db in
                if ZSettingKit.shared.role == .anchor {
                    let row = try Row.fetchOne(db, sql: "SELECT COUNT(t1.message_id) as unReadCount FROM tb_message t1 LEFT JOIN tb_message_user t2 ON t1.message_userid = t2.userid WHERE t1.login_userid = ? AND t1.message_read_state = 0 AND t2.role != ?", arguments: [loginuserid, ZSettingKit.shared.role])
                    count = Int(row?["unReadCount"] as? Int64 ?? 0)
                } else {
                    let row = try Row.fetchOne(db, sql: "SELECT COUNT(t1.message_id) as unReadCount FROM tb_message t1 LEFT JOIN tb_message_user t2 ON t1.message_userid = t2.userid WHERE t1.login_userid = ? AND t1.message_read_state = 0 ", arguments: [loginuserid])
                    count = Int(row?["unReadCount"] as? Int64 ?? 0)
                }
            })
        } catch {
    ...
    public static func getArrayMessageRecord<T: ZModelMessageRecord>(models: inout [T]?, page: Int) {
        do {
            let loginuserid = ZSettingKit.shared.userId
            if ZSettingKit.shared.role == .anchor {
                let sql = "SELECT t1.*, (SELECT COUNT(*) FROM tb_message t3 WHERE t3.message_userid = t1.userid AND t3.message_read_state = 0) AS message_unread_count FROM tb_message_user t1 WHERE t1.login_userid = ? AND t1.role != ? GROUP BY t1.userid ORDER BY t1.role DESC, t1.message_time DESC LIMIT \(kPageCount) OFFSET \((page - 1) * kPageCount)"
                try ZSQLiteKit.shared.connection?.write({ db in
                    models = try T.fetchAll(db, sql: sql, arguments: [loginuserid, ZSettingKit.shared.role])
                })
            } else {
                let sql = "SELECT t1.*, (SELECT COUNT(*) FROM tb_message t3 WHERE t3.message_userid = t1.userid AND t3.message_read_state = 0) AS message_unread_count FROM tb_message_user t1 WHERE t1.login_userid = ? GROUP BY t1.userid ORDER BY t1.role DESC, t1.message_time DESC LIMIT \(kPageCount) OFFSET \((page - 1) * kPageCount)"
                try ZSQLiteKit.shared.connection?.write({ db in
